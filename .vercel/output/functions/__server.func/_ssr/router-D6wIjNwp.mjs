...
async function enrichPorndiff(hit) {
	const candidates = [`https://cdn.porndiff.com/tubes/${hit.slug}_poster_01.jpg`, `https://cdn.porndiff.com/tubes/${hit.slug}.jpg`];
...
function performerFromPsmcdn(url) {
	const m = url.match(/\/teamskeet\/[a-z]+\/([a-z0-9_]+)\//i);
	if (!m) return [];
	const name = m[1].replace(/\d+$/, "").split("_").filter(Boolean).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
	return name ? [name] : [];
}
